...
class RetinaNetC(nn.Module):
    ...
    def forward(self, batched_inputs: List[Dict[str, Tensor]]):
        ...
        # class head
        image_logits = self.image_cls_head(features[0]) # P3
        ...
        if self.training:
            ...
            # image cls
            gt_image_labels = [x["image_label"].to(self.device) for x in batched_inputs]
            image_loss = self.cls_loss(image_logits, gt_image_labels)
            losses["loss_image"] = 1.0 * image_loss

            """
            if wsi_data:
                wsi_patch_features = self.cls_features(wsi_data['images'])
                wsi_patch_features = wsi_patch_features.unsqueeze(0) # [B, N, C]
                wsi_logits = self.wsi_logits(wsi_patch_features)  # [N, K]
                wsi_targets = wsi_data['label'].unsqueeze(0).to(self.device) # [N, K]
                wsi_cls_loss = F.binary_cross_entropy_with_logits(wsi_logits, wsi_targets, reduction='mean')
                losses['wsi_cls_loss'] = wsi_cls_loss                
            """
            ...
            if torch.jit.is_scripting():
                return results
            processed_results = []
            for results_per_image, input_per_image, image_size, image_logit in zip(
                results, batched_inputs, images.image_sizes, image_logits
            ):
                height = input_per_image.get("height", image_size[0])
                width = input_per_image.get("width", image_size[1])
                r = detector_postprocess(results_per_image, height, width)
                processed_results.append({"instances": r, "image_logit": image_logit.sigmoid()})
...
class ImageClsHead(nn.Module):
    ...
    def __init__(
        ...
        subnet.append(nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=2, padding=1))
        if norm:
            subnet.append(get_norm(norm, out_channels))

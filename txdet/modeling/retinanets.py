...
class RetinaNetS(nn.Module):
    """
    Implement `RetinaNet`, but use softmax + Hard Negative Sampling.
    """
        ...
        *,
        ...
        Args:
            ...
            positive_fraction (float): a number between 0 and 1 which indicates the proportion of positive
                proposals used during the training of the classification head. It is used to estimate 
        ...
        if len(self.backbone.output_shape()) != len(self.head_in_features):
            logger.warning("[RetinaNetS] Backbone produces unused features.")
        ...
        self.num_classes = num_classes # include bg class
        # Loss parameters:
    ...
    def losses(self, anchors, pred_logits, gt_labels, pred_anchor_deltas, gt_boxes):
        ...
        num_foreground = pos_mask.sum().item()
        get_event_storage().put_scalar("num_foreground", num_foreground / num_images)
        self.loss_normalizer = self.loss_normalizer_momentum * self.loss_normalizer + (
            1 - self.loss_normalizer_momentum
        ) * max(num_foreground, 1)

        # classification loss
        pred_logits = cat(pred_logits, dim=1)[valid_mask].view(-1, self.num_classes)
        gt_classes = gt_labels[valid_mask].view(-1)
        cls_loss_all = F.cross_entropy(pred_logits, gt_classes, reduction='none', label_smoothing=0.1)

        pos_inds = (gt_classes < (self.num_classes - 1)).nonzero(as_tuple=True)[0]
        neg_inds = (gt_classes == (self.num_classes - 1)).nonzero(as_tuple=True)[0]

        # Hard Negative Sampling in batch
        num_negative = max(10, int(3 * pos_mask.sum().item()))
        if num_negative > neg_inds.size(0):
            num_negative = neg_inds.size(0)
        topk_loss_cls_neg, _ = cls_loss_all[neg_inds].topk(num_negative)
        loss_cls_pos = cls_loss_all[pos_inds].sum()
    ...
    def label_anchors(self, anchors, gt_instances):
        ...
        for gt_per_image in gt_instances:
            ...
            if len(gt_per_image) > 0:
                ...
                gt_labels_i[anchor_labels == 0] = self.num_classes - 1 # the last id is backgroud class
    ...
    def inference_single_image(
        ...
        for box_cls_i, box_reg_i, anchors_i in zip(box_cls, box_delta, anchors):
            # (HxWxAxK,)
            predicted_prob = F.softmax(box_cls_i.view(-1, self.num_classes), dim=-1)[:, :-1] # remove backgroud

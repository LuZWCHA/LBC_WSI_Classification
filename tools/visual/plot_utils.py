...
def plot_many(image_list, titles=None, rows=None, cols=None, show=True, default_type="image", hist_params={}, **kwargs):
    ...
    layout_flag = False
    if rows is not None:
        cols =  ceil(image_num / rows)
    elif cols is not None:
        rows = ceil(image_num / cols)
    ...
    for i, image in enumerate(image_list):
        ...
        if default_type=="hist" or (image.ndim == 1 and default_type=="image"):
            default_type = "hist"
            plt.hist(image,  **hist_params, **kwargs)
...
def plot_confusion_matrix(matrix, nc, normalize=True, save_dir='', names=()):

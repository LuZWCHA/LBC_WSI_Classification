...
from txdet.data.dataset_mapper_contrast import DatasetMapperContrast
from txdet.data.dataset_mapper import DatasetMapper
# from WSI_framework.detectron.configs.common.data.coco import dataloader
from exp.retinanet_mil_contrast_2view import RetinaNetMCL, RetinaNetHead, ImageClsHead
from exp.mil import MILClassifier


########## model ############
model = L(RetinaNetMCL)(
    backbone=L(FPN)(
        bottom_up=L(ResNet)(
...
dataloader.train = L(build_detection_train_loader)(
    dataset=L(get_detection_dataset_dicts)(names="abp_tct_c4_train", filter_empty=False),
    mapper=L(DatasetMapperContrast)(
        is_train=True,
        augmentations=[
...
dataloader.test = L(build_detection_test_loader)(
    dataset=L(get_detection_dataset_dicts)(names="abp_tct_c4_test", filter_empty=False),
...
########## train ############
train = dict(
    output_dir="./output-contrast-2view-samplemax",
    init_checkpoint="/nasdata/share/abp_trainer/trained_models/retinanet_mil_211103/model_0119999.pth",
    max_iter=60000,
    ...
    checkpointer=dict(period=1000, max_to_keep=50),  # options for PeriodicCheckpointer
    eval_period=60000,
    log_period=20,
    device="cuda"
    # ...

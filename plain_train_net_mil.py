...
import datasets
from exp.wsi_dataset import WSIDataset, build_wsi_test_dataloader, build_wsi_train_dataloader
...
def do_test(cfg, model):
    # wsi dataset
    test_file = 'testset_sub3_640_npy_3N.txt'
    ...
    if comm.is_main_process():
        ...
        df.to_csv(os.path.join(cfg.train.output_dir, Path(test_file).stem+ ".csv"), index=False)
    model.train()
...
def do_train(args, cfg):
    ...
    dataset = WSIDataset(train_file, size=(640, 384), train=True, 
        albu_transform=albu_transform)
    wsi_dt = build_wsi_train_dataloader(dataset, 4, num_workers=4)
    ...
    with EventStorage(start_iter) as storage:
        ...
        for iteration in range(start_iter, max_iter):
                ...
                with autocast(enabled=cfg.train.amp.enabled):
                    loss_dict = model(data, wsi_data=wsi_data[0])
